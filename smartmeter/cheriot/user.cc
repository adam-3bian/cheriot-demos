...
int user_data_entry()
{
	...
	static constexpr size_t nEvents = 5;
	...
	MultiWaiter              mw;
	struct EventWaiterSource events[nEvents] = {
	  {&sensorData->version, 0},
...
constexpr std::string_view jsTopic{"cheriot-smartmeter/u/js"};
                                      ...
                                      size_t      payloadLength)
{
	...
	if (topicView == jsTopic)
	{
		Debug::log("Got js PUBLISH");
		user_javascript_load(static_cast<const uint8_t *>(payload),
		                     payloadLength);
	}
	else
	{
