...
constexpr std::string_view scheduleTopic{"cheriot-smartmeter/p/schedule"};
constexpr std::string_view varianceTopic{"cheriot-smartmeter/p/variance"};
constexpr std::string_view publishTopic{"cheriot-smartmeter/p/update"};
                                      ...
                                      size_t      payloadLength)
{
	...
	if (topicView == scheduleTopic)
	{
		...
		for (int i = 0; i < 24; i++)
		{
			payload.today_rate[i] = strtol(ptr, &ptr, 10);
		}
		for (int i = 0; i < 24; i++)
		{
			payload.tomorrow_rate[i] = strtol(ptr, &ptr, 10);
		}
	...
	else if (topicView == varianceTopic)
	{
		/*
